...
/**
 * A realm which decodes authorization data from <em>already authenticated</em> users from Active Directory
 * via <a href="https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-pac/166d8064-c863-41e1-9c23-edaaa5f36962">MS-PAC</a>.
 * <p>
 * This realm requires your JVM to provide an <a href="https://docs.oracle.com/javase/8/docs/jre/api/security/jgss/spec/com/sun/security/jgss/ExtendedGSSContext.html">{@code ExtendedGSSContext}</a>
 * implementation. It will use <a href="https://docs.oracle.com/javase/8/docs/jre/api/security/jgss/spec/com/sun/security/jgss/InquireType.html#KRB5_GET_AUTHZ_DATA">{@code InquireType.KRB5_GET_AUTHZ_DATA}</a>
 * to extract {@code AuthorizationData} according to RFC 4120, section 5.2.6 from an established
 * security context, and use the {@link Pac} parser to extract all relevant authorization data
 * (group SIDs), validate the PAC data server signature with the {@link PrivateSunPacSignatureVerifier}
 * and the supplied keytab (login context) and process the data according to
 * <a href="https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-kile/4ad7ed1f-0bfa-4b5f-bda3-fedbc549a6c0">MS-KILE, section 3.4.5.3</a>.
 *
 * <h2 id="configuration">Configuration</h2> Following options can be configured:
 * <ul>
 * <li>{@code loginEntryName}: the login entry identical to the one from {@link SpnegoAuthenticator#getLoginEntryName()}.</li>
 ...
 * {@code userPrincipalName} (if available), {@code msDS-PrincipalName}. Default is {@code false}.</li>
 * </ul>
 * <p>
 * <strong>Note:</strong> This realm is meant to be an alternative to the {@link ActiveDirectoryRealm}
 * when no more additional attributes or other role formats are required beyond those provided by
 * the PAC data and {@link SpnegoAuthenticator SPNEGO authentication} is used
 * ({@link SSLAuthenticator X.509 authentication} is not supported). Moreover, all processing
 * happens in memory, for that reason it is <em>orders of magnitude faster</em> than the previously
 * mentioned one.
 */
public class PacDataActiveDirectoryRealm extends ActiveDirectoryRealmBase {
	...
	protected Principal getPrincipal(GSSName gssName, GSSCredential gssCredential, GSSContext gssContext) {
		...
		if (gssContext instanceof ExtendedGSSContext) {
			...
			Optional<AuthorizationDataEntry> pacDataEntry = Arrays.stream(adEntries)
					...
					.map(adEntry -> AdIfRelevantAsn1Parser.parse(adEntry.getData())).flatMap(List::stream)
					.filter(adEntry -> adEntry.getType() == AdIfRelevantAsn1Parser.AD_WIN2K_PAC).findFirst();

			if (pacDataEntry.isPresent()) {
				Pac pac = new Pac(pacDataEntry.get().getData(), new PrivateSunPacSignatureVerifier());

				Key[] keys = getKeys();
				try {
					pac.verifySignature(keys);
				} catch (SignatureException e) {
					logger.warn(sm.getString("pacDataActiveDirectoryRealm.signatureVerificationFailed"), e);
					return null;
				}
				...
				if ((userAccountControl & USER_NORMAL_ACCOUNT) == 0L && (userAccountControl & USER_WORKSTATION_TRUST_ACCOUNT) == 0L) {
				...
				if (kerbValidationInfo.getExtraSids() != null) {
					...
					groups.addAll(kerbValidationInfo.getExtraSids().stream().skip(n).map(extraSid -> extraSid.getSid())
							.collect(Collectors.toList()));
				}
				if (kerbValidationInfo.getResourceGroupDomainSid() != null) {
					groups.addAll(kerbValidationInfo
							.getResourceGroupIds().stream().map(resourceGroupId -> kerbValidationInfo
									.getResourceGroupDomainSid().append(resourceGroupId.getRelativeId()))
							.collect(Collectors.toList()));
				}
				...
				List<String> roles = groups.stream().map(String::valueOf).map(group -> roleFormatPrefix + group)
						.collect(Collectors.toList());

				if (logger.isTraceEnabled())
					logger.trace(sm.getString("activeDirectoryRealm.foundRoles", roles.size(), gssName, roles));
				...
				if (logger.isDebugEnabled())
					logger.debug(sm.getString("pacDataActiveDirectoryRealm.noDataProvided", gssName));
			}
		} else {
	...
	protected Key[] getKeys() {
		...
		try {
			...
			throw new IllegalStateException("Failed to load Kerberos keys for login entry '" + loginEntryName + "'", e);
		} finally {
			if (lc != null) {
				try {
					lc.logout();
