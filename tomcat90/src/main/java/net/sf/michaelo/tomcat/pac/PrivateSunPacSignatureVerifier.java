...
/**
 * A PAC signature verifier based on private Sun classes from Java's Kerberos
 * implementation.
 * <p>
 * <strong>Note:</strong> This implementation is far from ideal because it uses private classes
 * which can break anytime. A better solution would be to use the
 * <a href="https://directory.apache.org/kerby/">Apache Kerby</a> library.
 */
public class PrivateSunPacSignatureVerifier extends PacSignatureVerifierBase {
	...
	protected void verifyInternal(PacSignatureData signatureData, byte[] data, Key[] keys) throws SignatureException {
		...
		String actualFailedSignaturesStr = actualFailedSignatures.stream().map(Base64.getEncoder()::encodeToString)
				.collect(Collectors.joining(",", "[", "]"));
		throw new SignatureException("Calculated signatures " + actualFailedSignaturesStr
				+ " do not match expected signature " + Base64.getEncoder().encodeToString(expectedSignature));
	}
