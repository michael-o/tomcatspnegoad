 ...
 * <p>
 * <strong>Important:</strong> It is imperative to pass a suitable signature verifier implementation and
 * the long term Kerberos keys for the principal from the keytab which were used to establish the
 * security context. The simplest implementation is the {@link PrivateSunPacSignatureVerifier} which
 * uses private Sun classes to perform the calculation.
 */
public class Pac {
	 ...
	 *
	 * @param pacDataBytes      PAC data structure encoded as bytes
	 * @param signatureVerifier a signature verifier implementation
	 * @throws NullPointerException     if {@code infoBytes} is null
	 * @throws IllegalArgumentException if {@code infoBytes} is empty
	 * @throws NullPointerException     if {@code signatureVerifier} is null
	 * @throws IllegalArgumentException if PAC version is not 0
	 * @throws IllegalArgumentException if an embedded {@code PAC_INFO_BUFFER}
	 *                                  structure offset is not a multiple of 8
	 * @throws IllegalArgumentException if any embedded structure is invalid
	 * @throws IllegalArgumentException if any of the required embedded structures
	 *                                  ({@code KERB_VALIDATION_INFO},
	 *                                  {@code PAC_CLIENT_INFO},
	 *                                  {@code PAC_SIGNATURE_DATA} (Server Signature),
	 *                                  {@code PAC_SIGNATURE_DATA} (KDC Signature))
	 *                                  is not present
	 */
	public Pac(byte[] pacDataBytes, PacSignatureVerifier signatureVerifier) {
		...
		for (long l = 0L; l < buffers; l++) {
			...
			buf.position(pos);
			if (logger.isTraceEnabled())
				logger.trace("PAC_INFO_BUFFER describes type " + String.format("0x%08X", type) + " with size "
						+ bufferSize + " and offset " + offset + " containing data " + Base64.getEncoder().encodeToString(data));
		...
		for (PacInfoBuffer pacInfoBuffer : pacInfoBuffers) {
			...
			if (type == KERB_VALIDATION_INFO) {
				...
				if (serverSignature != null) {
					if (logger.isTraceEnabled())
						logger.trace("Ignoring additional PAC_SIGNATURE_DATA (Server Signature) structure");
				} else {
					if (logger.isTraceEnabled())
				...
				if (kdcSignature != null) {
					if (logger.isTraceEnabled())
						logger.trace("Ignoring additional PAC_SIGNATURE_DATA (KDC Signature) structure");
				} else {
					if (logger.isTraceEnabled())
				...
				if (logger.isTraceEnabled())
					logger.trace("Ignoring unsupported structure type " + String.format("0x%08X", type) + " with data "
	...
	/**
	 * Verifies the server signature of this PAC data structure with zeroed server and KDC
	 * signature values with the supplied long term Kerberos keys.
	 *
	 * @param keys an array of long term Kerberos keys for the principal from the
	 *             keytab which was used to establish the security context
	 * @throws SignatureException if the signature validation fails with all
	 *                            supplied keys
