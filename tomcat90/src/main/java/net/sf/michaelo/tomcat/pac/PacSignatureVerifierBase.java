...
/**
 * A base implementation of the {@link PacSignatureVerifier}. Implementors are
...
public abstract class PacSignatureVerifierBase implements PacSignatureVerifier {
	...
	public void verify(PacSignatureData signatureData, byte[] data, Key[] keys) throws SignatureException {
		...
		Key[] filteredKeys = Arrays.stream(keys)
				.filter(key -> ETYPE_MAPPER.get(key.getAlgorithm()) != null
						&& ETYPE_MAPPER.get(key.getAlgorithm()) == signatureData.getType().getEType())
				.toArray(Key[]::new);
