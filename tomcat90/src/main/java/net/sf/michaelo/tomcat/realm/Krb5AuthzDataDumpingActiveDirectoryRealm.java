 ...
 * security context, dump to <code>{catalina.base}/work/KRB5_AUTHZ_DATA/{gssName}/{yyyyMMdd'T'HHmmss.SSS}{#n?}</code>
 * and continue as described.
 * <p>
 * <strong>Note</strong>: Use this realm for testing/analysis purposes only along with the {@link Krb5AuthzDataDumpPrinter}.
 */
public class Krb5AuthzDataDumpingActiveDirectoryRealm extends ActiveDirectoryRealm {

	private static final DateTimeFormatter TS_FORMAT = DateTimeFormatter.ofPattern("yyyyMMdd'T'HHmmss.SSS")
	...
	protected Principal getPrincipal(GSSName gssName, GSSCredential gssCredential, GSSContext gssContext) {
		if (gssContext instanceof ExtendedGSSContext) {
			...
			try {
				...
				logger.warn(
						sm.getString("krb5AuthzDataDumpingActiveDirectoryRealm.dumpingKrb5AuthzDataFailed", gssName),
						e);
			}
		} else {
